...
const codeToTextMapping: Record<string, string> = {
  ...
  customer_pickup_H: "Customer pickup",
...
function getFreightData(data: unknown) {
  ...
  const preferredCarrierValue = get(
    data,
    `${firstTransactionSet}.heading.carrier_details_routing_sequence_transit_time_TD5[0].transportation_method_type_code_04`
  );
  const preferredCarrier =
    codeToTextMapping[preferredCarrierValue] ?? preferredCarrierValue;
...
function CoreMarkFreight(props: Props) {
