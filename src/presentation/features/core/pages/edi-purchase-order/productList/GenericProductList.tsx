...
//This function caters to both 850(Inbound) and 855(Outbound)
function GenericProductList(props: Props) {
  //Inbound doc json have 'output' field present which contain the interchanges data e.g. { 'output': { 'interchanges': {...}}},
  //while outbound doc json have the interchanges data directly present { 'interchanges': {...} }
  const isInboundDocument = !!props.data?.output;

  const firstTransactionSetData: TransactionSet | undefined = isInboundDocument
    ? get(props.data, `${firstTransactionSet}`)
    : get(props.data, `${firstTransactionSetNoOutput}`);

  const heading = firstTransactionSetData?.heading;
  const detail = firstTransactionSetData?.detail;
  const summary = firstTransactionSetData?.summary;
