...
function getProductsData(data: unknown) {
  ...
  const productData = productList.map((item) => {
    ...
    const buyerPart = get(
      item,
      `baseline_item_data_invoice_IT1.product_service_id_07`,
      ""
    ) as string;
    ...
    const service = get(
      item,
      `baseline_item_data_invoice_IT1.product_service_id_11`,
      ""
    ) as string;
    ...
    <>
          ...
          {products.length
            ? products.map((product) => (
                ...
                >
                  ...
                  <td>{product.uom}</td>
                  <td>
                    {(product.orderQuantity * product.purchasePrice).toFixed(2)}
                  </td>
                </tr>
              ))
            : null}
      ...
      <ItemTable>
        <thead>
          <tr>
            <td>
              Total Qty: {summary.totalQty} {summary.uom}
            </td>
          </tr>
        </thead>
      </ItemTable>
