...
export const getInvoiceNumber = (data: unknown) => {
...
function getSummaryTableData(data: unknown) {
  ...
  const purchaseOrderDate = get(
    data,
    `${firstTransactionSet}.heading.beginning_segment_for_invoice_BIG.date_03`,
    ""
  );
  ...
  return {
    ...
    purchaseOrderDate: formatDate(purchaseOrderDate, "MM/DD/YYYY"),
...
function GenericInvoiceReadViewHeader({ data }: { data: unknown }) {
      ...
      >
        <HeaderSummaryTable summary="These are the key and value pairs">
          <tr>
            ...
            <td>PO date:</td>
            <br /> <strong>{summaryData.purchaseOrderDate}</strong>
          </tr>
          <tr>
            <th scope="row">
              Ship date:
...
export { GenericInvoiceReadViewHeader };
