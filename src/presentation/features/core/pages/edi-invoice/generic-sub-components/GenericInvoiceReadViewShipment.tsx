...
function getShippingInformation(data: unknown) {
  ...
  const shipToReference =
    ...
    find(identificationLoop, {
      party_identification_N1: { entity_identifier_code_01: "ST" },
    }) ||
    find(identificationLoop, {
  ...
  const billToReference =
    ...
    find(identificationLoop, {
      party_identification_N1: { entity_identifier_code_01: "BT" },
    }) ||
    find(identificationLoop, {
      name_N1: {
        entity_identifier_code_01: "bill_to_party_BT",
  ...
  const remitToReference = find(identificationLoop, (item) => {
    const nameNode = item?.name_N1 ?? item?.party_identification_N1;
    const identifierCode = nameNode?.entity_identifier_code_01;
    const result = identifierCode === "RE";
    return result;
  });
...
function ShippingAddress({ title, data }: { title: string; data: unknown }) {
  ...
  const geographicLocationInfo = get(data, "geographic_location_N4") as Record<
    string,
    string
  >;
...
function GenericInvoiceReadViewShipment({ data }: { data: unknown }) {
  const shippingInfo = getShippingInformation(data);
  return (
    <GenericShipToContainer>
      {shippingInfo.shipToReference ? (
        <ShippingAddress title="Ship to:" data={shippingInfo.shipToReference} />
      ) : null}
      {shippingInfo.billToReference ? (
        <ShippingAddress title="Bill to:" data={shippingInfo.billToReference} />
      ) : null}
      {shippingInfo.remitToReference ? (
        <ShippingAddress
          title="Remit to:"
          data={shippingInfo.remitToReference}
        />
      ) : null}
    </GenericShipToContainer>
  );
...
export { GenericInvoiceReadViewShipment };
