...
import { getCodeLabelFromOptions } from "./GenericSubFormTableSACInvoiceReadViewFreight";
...
function getProductsData(data: unknown) {
  ...
  const productData = productList.map((item) => {
    ...
    const { sku: buyerPart, upc: service, vendor } = getLineItemsFromObject(
    ...
    return {
      lineNumber,
      buyerPart,
      vendorPart: vendorPart ? vendorPart : vendor,
      service,
      productDescription,
      purchasePrice,
      retailPrice,
...
function GenericInvoiceReadViewProductList({
  data,
  additionalData,
}: {
  data: unknown;
            ...
            {products.length
              ? products.map((product) => (
                  ...
                  >
                    ...
                    <td>{currencyUSDFormatter(product.purchasePrice)}</td>
...
export { GenericInvoiceReadViewProductList };
