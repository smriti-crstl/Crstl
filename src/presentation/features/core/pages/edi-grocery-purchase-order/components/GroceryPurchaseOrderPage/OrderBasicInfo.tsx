...
export const OrderBasicInfo: React.FC<PublicProps> = ({ data }) => {
  ...
  const poNumber = get(
    data,
    `${firstTransactionSet}.heading.purchase_order_identification_G50.purchase_order_number_03`
  );
  const orderStatusCode = get(
    data,
    `${firstTransactionSet}.heading.purchase_order_identification_G50.order_status_code_01`
  );
